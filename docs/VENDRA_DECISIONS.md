...
`main` es la rama estable, validada y publicable; no se usa para desarrollo directo. `codex/diagnostico-ux` es la rama permanente de integración para todo el trabajo realizado con Codex. Las ramas de tarea son temporales, nacen desde `codex/diagnostico-ux` actualizada y, después de revisión, validación, publicación remota e integración comprobada, se eliminan local y remotamente con autorización explícita. Al completar una tarea o conjunto coherente de tareas, `codex/diagnostico-ux` se integra en `main`, preferentemente mediante fast-forward, y `main` se valida antes de subirla. Solamente `main` y `codex/diagnostico-ux` deben permanecer como ramas permanentes. Los tags se reservan para puntos estables o versiones relevantes.
